...
class Getdata:
    def __init__(self, active_leagues=None, all_categories=None):
        if active_leagues is None:
            active_leagues = {
                'Standard': 'Standard', 'Hardcore': 'Hardcore'
            }  # постоянно действующие лиги
    ...
    def go_url(self, league, category):
        urls = f'https://api.poe.watch/get?' \
               f'league={self.get_current_leagues()[league]}' \
    ...
    def get_categories(self):
        """форимирование словаря с категориями + убираем ненужные категории"""
        unnecessary_categories = (
            'gem', 'base', 'prophecy', 'enchantment', 'beast', 'net',
            'vial', 'splinter', 'currency', 'essence', 'piece', 'catalyst',
            'influence', 'watchstone', 'map'
        )
    ...
    def parse(self, league, category):
        """выборка конкретной категории в конкретной лиге"""
        ...
        df = pd.DataFrame(self.go_url(league, category),
                          columns=['type', 'mean'])
    ...
    def make_name_list(self, league, category):  # выборка по имени предмета
        items = dict()
        response = requests.get(f'https://api.poe.watch/get?'
                                f'league={self.get_current_leagues()[league]}'
                                f'&category=map').json()
    ...
    def currency_check(self, category):
        """является ли выбранная категория подкатегорией в currency"""
        currency_categories = (
            'fossil', 'resonator', 'incubator', 'oil', 'fragment'
        )
    ...
    def make_currency_list(self, league, category):
        """выборка предметов подкатегории в currency"""
        response = requests.get(f'https://api.poe.watch/get?'
                                f'league={self.get_current_leagues()[league]}'
    ...
    def parse_all(self, league):
        """выборка по ВСЕМ категориям, полученным из self.get_categories()"""
        itemlist = dict()
